...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sign a document with a Cloud KMS post-quantum key (ML-DSA-65)."
    )
    ...
    parser.add_argument("--input",    required=True, help="Path to the document to sign")
    parser.add_argument("--output",   required=True, help="Path to save the .sig file")
...
def sign_document(
    ...
    print(f"[INFO] Signature saved to: {output_path} ({len(sign_response.signature)} bytes)")

    if is_digest:
        print("[NOTE] Large file: signature covers the SHA-512 digest of the document.")
        print("[NOTE] Use verify.py with the original file — it applies the same digest logic.")
